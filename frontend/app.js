    const API = (location.protocol === 'file:' || (['localhost', '127.0.0.1'].includes(location.hostname) && location.port !== '5000'))
    ...
    window.addEventListener('load', async () => {
      ...
      if (br.success && br.data) {
        ...
        const ph = document.querySelector('.policy-header h1');
      ...
      // Otherwise show auth screen (default — already shown)
    });
    ...
    async function loadStoreData() {
      ...
      if (cartCnt() > 0) {
        setTimeout(() => {
          if (cartCnt() > 0 && document.querySelector('.page.active')?.id !== 'page-cart') {
            toast('🛒 You have items waiting in your cart! Don\'t miss out.');
          }
        }, 15000); // Remind after 15 seconds for demo, usually 5-10 mins
      }
    ...
    function renderCatFull() { document.getElementById('catFull').innerHTML = S.categories.map(c => `<div class="pgc" onclick="filterCat('${c._id || c.name}')"><div class="pgc-img"><img src="${c.image || CIMG[c.name] || PIMG[0]}" alt="${c.name}" loading="lazy" onerror="this.src='https://via.placeholder.com/140/181818/FF4500?text=📦'"></div><div class="pgc-body"><h4>${c.name}</h4><span style="font-size:.63rem;color:var(--o)">Browse →</span></div></div>`).join('') }
    ...
    <div class="pd-desktop-grid">
      ...
      <div class="pd-col-info">
        ...
        <div style="height:40px" class="pd-mobile-spacer"></div>
      </div>
      ...
      <div class="pd-col-buy">
        <div class="pd-qty-row">
          ...
          <div class="pd-qc">
            <button class="pd-qb" onclick="pdChQty(-1)"><i class="fas fa-minus"></i></button>
            <span class="pd-qn" id="pdQtyNum">1</span>
            <button class="pd-qb" onclick="pdChQty(1)"><i class="fas fa-plus"></i></button>
          </div>
        </div>
        <div class="pd-acts">
          <button class="pd-ac" onclick="pdAddCart()"><i class="fas fa-cart-plus"></i> Add to Cart</button>
          <button class="pd-by" onclick="pdBuyNow()" ${p.stock === 0 ? 'disabled style="opacity:.5"' : ''}><i class="fas fa-bolt"></i> Buy Now</button>
        </div>
      </div>
    </div>
    ...
    <div class="related-section">
      <div class="related-head"><i class="fas fa-magic"></i> You Might Also Like</div>
      <div class="related-grid" id="relatedGrid">
        ${S.products.filter(x => x.category?._id === p.category?._id || x.category === p.category).filter(x => x._id !== p._id).slice(0, 4).map(pgcCard).join('')}
      </div>
    </div>
    `;
    ...
    function renderAcc() {
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:20px">
        <div class="pcard" style="padding:15px 10px; text-align:center; border:1px solid #FFD700">
          <i class="fas fa-coins" style="color:#FFD700;font-size:1.2rem;margin-bottom:5px"></i>
          <div style="font-size:.6rem;color:var(--g)">RK COINS</div>
          <div style="font-size:.9rem;font-weight:900;color:#FFD700">${(S.user.walletBalance || 0).toFixed(0)}</div>
        </div>
        <div class="pcard" onclick="go('wishlist',document.getElementById('bn-wishlist'))" style="cursor:pointer;padding:15px 10px; text-align:center; border:1px solid var(--o)">
          <i class="fas fa-heart" style="color:var(--o);font-size:1.2rem;margin-bottom:5px"></i>
          <div style="font-size:.6rem;color:var(--g)">SAVED</div>
          <div style="font-size:.9rem;font-weight:900;color:var(--w)">${S.wish.size}</div>
        </div>
        <div class="pcard" onclick="go('cart',document.getElementById('bn-cart'))" style="cursor:pointer;padding:15px 10px; text-align:center; border:1px solid var(--green)">
          <i class="fas fa-shopping-bag" style="color:var(--green);font-size:1.2rem;margin-bottom:5px"></i>
          <div style="font-size:.6rem;color:var(--g)">IN CART</div>
          <div style="font-size:.9rem;font-weight:900;color:var(--w)">${cartCnt()}</div>
        </div>
      </div>
        ...
        return `<div class="ocard" id="order-${o._id}">
          ...
          <div class="order-timeline">
            ${['Ordered', 'Confirmed', 'Shipped', 'Delivered'].map((step, idx) => {
              ...
              const currentStatusIdx = statusMap[o.orderStatus?.toLowerCase()] || 0;
              const isActive = idx <= currentStatusIdx;
              const icons = ['fa-shopping-cart', 'fa-check-circle', 'fa-truck', 'fa-hand-holding-heart'];
              return `
                <div class="ot-step ${isActive ? 'active' : ''}">
                  <div class="ot-dot"><i class="fas ${icons[idx]}"></i></div>
                  <div class="ot-label">${step}</div>
                </div>`;
            }).join('')}
          </div>
