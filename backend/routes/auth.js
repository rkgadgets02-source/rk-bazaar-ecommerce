    ...
    html: `
      <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #eee">
        <div style="background:linear-gradient(135deg,#FF4500,#FF6B00);padding:30px 24px;text-align:center">
          ...
          <p style="color:rgba(255,255,255,.85);margin:8px 0 0;font-size:13px">Premium Store</p>
        </div>
...
router.post('/register', async (req, res) => {
  try {
    ...
    res.status(201).json({
      success: true,
      message: emailSent
        ? `OTP sent to ${email}. Please verify to activate your account.`
        : `Account created! OTP email could not be sent. Please use "Resend OTP" to try again.`,
      email,
    });
  } catch (err) {
        ...
        html: `
          <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto">
            ...
            <div style="padding:28px 24px;background:#fff;border:1px solid #eee;border-top:none;border-radius:0 0 16px 16px">
              ...
              <p style="color:#666">Your account has been verified successfully. Start shopping premium products at unbeatable prices!</p>
